(ns kinaxis.main
  "Kotodama WASM entrypoint for the Kinaxis clean-room actor (L4) — Clojure port.
  ...
  facts (`kinaxis.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kinaxis")
(def tier "L4")
...
(def entity-specs
  [{:entity "Catalog"       :plural "catalogs"       :id-prefix "kinaxis_cat"
    :fields [:name :supplierId :itemCount]          :required [:name :itemCount]
    :coerce {:itemCount :int}                  :refs {:supplierId "Supplier"}}
   {:entity "Product"       :plural "products"       :id-prefix "kinaxis_pro"
    :fields [:sku :title :price :currency]          :required [:sku :title]
    :coerce {:price :float}                    :refs {}}
   {:entity "Requisition"   :plural "requisitions"   :id-prefix "kinaxis_req"
    :fields [:requesterId :total :status]           :required [:total :status]
    :coerce {:total :float}                    :refs {}}
   {:entity "PurchaseOrder" :plural "purchaseorders" :id-prefix "kinaxis_pur"
    :fields [:supplierId :total :status]            :required [:total :status]
    :coerce {:total :float}                    :refs {:supplierId "Supplier"}}
   {:entity "Supplier"      :plural "suppliers"      :id-prefix "kinaxis_sup"
    :fields [:name :taxId :rating]                  :required [:name :rating]
    :coerce {:rating :float}                   :refs {}}
   {:entity "Invoice"       :plural "invoices"       :id-prefix "kinaxis_inv"
    :fields [:supplierId :poId :total :status]      :required [:total :status]
    :coerce {:total :float}                    :refs {:supplierId "Supplier"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"kinaxis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kinaxis-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kinaxis-compat/ready)
